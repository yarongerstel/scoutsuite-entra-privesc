> **Fork notice**: this repository is a fork of [nccgroup/ScoutSuite](https://github.com/nccgroup/ScoutSuite) (based on release `5.14.0`, the latest at fork time), extended with additional Azure AD / Entra ID privilege-escalation checks, Azure network segregation checks, and AWS check improvements. See [`docs/entra-privesc-checks.md`](docs/entra-privesc-checks.md) for the Entra checks, [`docs/network-segregation-checks.md`](docs/network-segregation-checks.md) for the cross-subscription/environment network checks, [`docs/DEVELOPMENT-NOTES.md`](docs/DEVELOPMENT-NOTES.md) for a handoff summary, how-to-test, the "add a new check" pattern, and the backlog of follow-up ideas, and [`docs/SESSION-STATE.md`](docs/SESSION-STATE.md) for a continuity snapshot (what was built, real bugs found/fixed, and any open decision awaiting an answer) - read that one first if picking this up cold.
