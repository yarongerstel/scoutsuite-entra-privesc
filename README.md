> **Fork notice**: this repository is a fork of [nccgroup/ScoutSuite](https://github.com/nccgroup/ScoutSuite) (based on release `5.14.0`, the latest at fork time), extended with additional Azure AD / Entra ID privilege-escalation checks and AWS check improvements. See [`docs/entra-privesc-checks.md`](docs/entra-privesc-checks.md) for the added checks, required Graph API permissions, and known limitations, and [`docs/DEVELOPMENT-NOTES.md`](docs/DEVELOPMENT-NOTES.md) for a handoff summary, how-to-test, the "add a new check" pattern, and the backlog of follow-up ideas.
